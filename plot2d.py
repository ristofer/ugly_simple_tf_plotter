...
aruco = np.loadtxt("datos_aruco_frame_camara3.txt")
odom = np.loadtxt("datos_odometria_frame_camara3.txt")
orb = np.loadtxt("datos_orb_frame_camara3.txt")
...
aruco_y = aruco[:,2]
aruco_z = aruco[:,3]
aruco_x = scipy.signal.medfilt(aruco_x,kernel_size=1011)
aruco_y = scipy.signal.medfilt(aruco_y,kernel_size=1011)
aruco_z = scipy.signal.medfilt(aruco_z,kernel_size=1011)
plt.figure(1)
plt.subplot(411)
